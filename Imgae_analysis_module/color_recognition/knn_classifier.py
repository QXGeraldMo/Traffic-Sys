...
def kNearestNeighbors(training_feature_vector, testInstance, k):
    ...
    # Calculate the actual distance from the training set to the test set for each number of training sets
    ...
    # Arrange the distance from small to large
    distances.sort(key=operator.itemgetter(1))
    neighbors = []
    # Take the first k distances after sorting
    for x in range(k):
...
def responseOfNeighbors(neighbors):
    ...
    # Count the number of each color category
    for x in range(len(neighbors)):
        ...
        if response in all_possible_neighbors:
            ...
            all_possible_neighbors[response] = 1
    # Sort the number of colors in descending order
